#!/usr/bin/env python3
"""
Populate database with AI-analyzed influencer data for frontend testing
"""
import os
import sys
import uuid
...
def main():
    """Main function to populate database with AI data"""
    try:
        print("🚀 Populating database with AI-analyzed influencer data...")
        ...
        if result.data:
            print(f"✅ Successfully added {len(result.data)} AI-analyzed influencers!")
            print("\n📊 Added influencers:")
            for influencer in result.data:
                alignment_score = influencer.get('audience_demographics', {}).get('buyer_alignment_score', 0)
                print(f"  - {influencer['name']} ({influencer['platform']}) - {alignment_score}/100 alignment")
        else:
            print("❌ Failed to add influencers to database")
