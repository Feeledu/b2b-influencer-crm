-- Migration: Enhance Campaigns Schema
-- Created: 2025-01-14
-- Description: Add missing fields to campaigns table to support full UI functionality
...
-- Add campaign goals and targeting fields
...
-- First, let's get a sample user ID (assuming there's at least one user)
DO $$
DECLARE
    sample_user_id UUID;
BEGIN
    -- Get the first user ID, or create a demo user if none exists
    SELECT id INTO sample_user_id FROM users LIMIT 1;
    
    IF sample_user_id IS NULL THEN
        -- Create a demo user if none exists
        INSERT INTO users (id, email, name, subscription_status) 
        VALUES (gen_random_uuid(), 'demo@fluencr.com', 'Demo User', 'pro')
        RETURNING id INTO sample_user_id;
    END IF;

    -- Insert sample campaigns
    INSERT INTO campaigns (
        id, user_id, name, description, status, start_date, end_date, budget, spent,
    ...
    -- Create some sample campaign-influencer relationships
    -- First, get some sample influencer IDs
    DECLARE
        influencer1_id UUID;
        influencer2_id UUID;
        influencer3_id UUID;
        ...
        campaign3_id UUID;
    BEGIN
        -- Get sample influencer IDs
        SELECT id INTO influencer1_id FROM influencers LIMIT 1 OFFSET 0;
        SELECT id INTO influencer2_id FROM influencers LIMIT 1 OFFSET 1;
        SELECT id INTO influencer3_id FROM influencers LIMIT 1 OFFSET 2;
        
        -- Get sample campaign IDs
        SELECT id INTO campaign1_id FROM campaigns WHERE name = 'Q4 SaaS Podcast Outreach' LIMIT 1;
        SELECT id INTO campaign2_id FROM campaigns WHERE name = 'Newsletter Sponsorship Series' LIMIT 1;
        SELECT id INTO campaign3_id FROM campaigns WHERE name = 'Tech Twitter Influencer Campaign' LIMIT 1;
        
        -- Insert campaign-influencer relationships if we have the IDs
        IF influencer1_id IS NOT NULL AND campaign1_id IS NOT NULL THEN
            INSERT INTO campaign_influencers (
                campaign_id, influencer_id, user_id, utm_source, utm_medium, utm_campaign,
                utm_content, utm_term, utm_url, status, notes, created_at, updated_at
            ) VALUES 
            (campaign1_id, influencer1_id, sample_user_id, 'influencer', 'podcast', 'q4-saas', 
             'q4_saas_podcast_outreach', 'saas', 
             'https://fluencr.com/campaign/q4-podcast?utm_source=influencer&utm_medium=podcast&utm_campaign=q4-saas&utm_content=q4_saas_podcast_outreach&utm_term=saas',
             'active', 'Primary podcast partner for Q4 campaign', NOW(), NOW())
            ON CONFLICT (campaign_id, influencer_id) DO NOTHING;
        END IF;
        
        IF influencer2_id IS NOT NULL AND campaign2_id IS NOT NULL THEN
            INSERT INTO campaign_influencers (
                campaign_id, influencer_id, user_id, utm_source, utm_medium, utm_campaign,
                utm_content, utm_term, utm_url, status, notes, created_at, updated_at
            ) VALUES 
            (campaign2_id, influencer2_id, sample_user_id, 'influencer', 'newsletter', 'sponsorship-series',
             'newsletter_sponsorship_series', 'b2b',
             'https://fluencr.com/campaign/newsletter-series?utm_source=influencer&utm_medium=newsletter&utm_campaign=sponsorship-series&utm_content=newsletter_sponsorship_series&utm_term=b2b',
             'completed', 'Newsletter sponsorship completed successfully', NOW() - INTERVAL '1 month', NOW() - INTERVAL '1 month')
            ON CONFLICT (campaign_id, influencer_id) DO NOTHING;
        END IF;
        
        IF influencer3_id IS NOT NULL AND campaign3_id IS NOT NULL THEN
            INSERT INTO campaign_influencers (
                campaign_id, influencer_id, user_id, utm_source, utm_medium, utm_campaign,
                utm_content, utm_term, utm_url, status, notes, created_at, updated_at
            ) VALUES 
            (campaign3_id, influencer3_id, sample_user_id, 'influencer', 'twitter', 'tech-twitter',
             'tech_twitter_campaign', 'tech',
             'https://fluencr.com/campaign/tech-twitter?utm_source=influencer&utm_medium=twitter&utm_campaign=tech-twitter&utm_content=tech_twitter_campaign&utm_term=tech',
             'active', 'Active Twitter influencer partnership', NOW() - INTERVAL '2 weeks', NOW() - INTERVAL '1 day')
            ON CONFLICT (campaign_id, influencer_id) DO NOTHING;
        END IF;
    END;
END $$;
...
COMMENT ON INDEX idx_campaigns_status IS 'Fast filtering by campaign status';
COMMENT ON INDEX idx_campaigns_progress IS 'Fast sorting by campaign progress';
COMMENT ON INDEX idx_campaigns_roi IS 'Fast sorting by ROI';
COMMENT ON INDEX idx_campaigns_revenue IS 'Fast sorting by revenue';
COMMENT ON INDEX idx_campaigns_start_date IS 'Fast filtering by start date';
COMMENT ON INDEX idx_campaigns_end_date IS 'Fast filtering by end date';
COMMENT ON INDEX idx_campaigns_utm_source IS 'Fast filtering by UTM source';
COMMENT ON INDEX idx_campaigns_utm_medium IS 'Fast filtering by UTM medium';
COMMENT ON INDEX idx_campaigns_utm_campaign IS 'Fast filtering by UTM campaign';
